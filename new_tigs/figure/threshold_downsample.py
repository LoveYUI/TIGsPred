...
for i in range(8,93):
    ...
    train_x_c,train_y_c = ros.fit_resample(train_x,train_y_c)
    model_svc = SVC(C=3, gamma=0.08838834764831845, kernel='rbf')
    model_svc.fit(train_x_c,train_y_c)
...
np.save("con_matrix_ds.npy",np.array(con_matrix))

con_matrix = np.load("con_matrix_ds.npy")
accs = []
precisions = []
recalls = []
