...
#acc
cls_result = np.load("cls_result75.npy",allow_pickle=True)
...
for i,j in enumerate(list(df.sort_values("acc", ascending=False, inplace=False)["acc"])):
    print(i,j)
    ax.text(i-0.3,j+0.003,str(round(j,3)),va='center',fontsize=15)
for i,j in enumerate(list(df.sort_values("acc", ascending=False, inplace=False)["precision"])):
    print(i,j)
    ax.text(i-0.1,j+0.003,str(round(j,3)),va='center',fontsize=15)
